package name.mkc1370.rhelper.generation;
...
public class TemplatesUtils {
	...
	public static String getTemplate(Project project, String name) {
		...
		if (file == null) {
			...
			if (file == null) {
				throw new RHelperException("Can't open template file(" + filename + ") after its creation");
			}
	...
	private static void createTemplateFromDefault(Project project, String name) {
		...
		try {
			defaultTemplate = getResourceContent("/name/mkc1370/rhelper/templates/" + filename);
		}
		catch (IOException e) {
